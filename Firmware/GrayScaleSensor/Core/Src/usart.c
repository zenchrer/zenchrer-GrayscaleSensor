...
/* USER CODE BEGIN 0 */
__IO uint8_t Usart1_TX_Buf[Usart1_TX_LEN]; //串口2发送数据数组
__IO uint8_t Usart1_RX_Buf[Usart1_RX_LEN]; //串口2接收数据数组
...
void UsartReceive_IDLE(UART_HandleTypeDef *huart) //串口接收空闲中断函数
{
    if ((__HAL_UART_GET_FLAG(huart, UART_FLAG_IDLE) != RESET))
    {
        ...
        if (huart->Instance == USART1)
        {
            ...
            HAL_UART_Receive_DMA(huart, (uint8_t *)Usart1_RX_Buf, Usart1_RX_LEN); //串口DMA接收
        }
    }
}
/* USER CODE END 1 */
